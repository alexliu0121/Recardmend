...
class ok_discount extends StatefulWidget {
  @override
...
class discount extends State<ok_discount> {
  int _currentIndex = 0;
...
class HomePage extends StatelessWidget {
  Future getData_taishin() async {
    ...
    QuerySnapshot qn =
        await firestore.collection('台新銀行').where('店家', isEqualTo: 'OK').get();
  ...
  Future getData_yushan() async {
    var firestore = FirebaseFirestore.instance;
    QuerySnapshot qn =
        await firestore.collection('玉山銀行').where('店家', isEqualTo: 'OK').get();
